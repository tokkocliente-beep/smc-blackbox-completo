...
conexoes_iq_conectando = set()  # ids de usuário com login em andamento em 2º plano
...
def obter_conexao_iq_do_usuario(usuario):
    ...
    BLOQUEIA a requisição até terminar — correto pra uma ação explícita da
    pessoa (ex: mandar uma ordem), onde faz sentido esperar."""
...
def obter_conexao_iq_sem_bloquear(usuario):
    """Versão que NUNCA deixa a requisição esperando o login (que pode
    levar vários segundos) — usada por coisas automáticas/repetidas (ex:
    consultar saldo a cada tanto tempo), pra nunca travar o resto do app
    enquanto conecta. Se já tem conexão pronta, devolve ela na hora. Se
    não tem ainda, dispara o login em outra thread (se não tiver uma
    rolando já) e devolve None — quem chamou mostra algo tipo
    "conectando..." e tenta de novo na próxima consulta, quando a conexão
    ...
    with conexoes_iq_lock:
        conexao = conexoes_iq_usuarios.get(usuario.id)
        if conexao is not None and conexao.check_connect():
            return conexao
        if usuario.id in conexoes_iq_conectando:
            return None
        conexoes_iq_conectando.add(usuario.id)

    usuario_id = usuario.id
    iq_email = usuario.iq_email
    senha = usuario.get_senha_iq()
    conta_tipo = usuario.iq_conta_tipo or "PRACTICE"

    def _conectar_em_fundo():
        try:
            if not senha:
                print("Conexão em 2º plano pro usuário", usuario_id, "cancelada: sem senha recuperável.")
                return
            nova = IQ_Option(iq_email, senha)
            check, motivo = nova.connect()
            if check:
                nova.change_balance(conta_tipo)
                with conexoes_iq_lock:
                    conexoes_iq_usuarios[usuario_id] = nova
            else:
                print("Falha ao conectar conta IQ do usuário", usuario_id, ":", motivo)
        except Exception as e:
            print("Erro ao conectar conta IQ em 2º plano:", e)
        finally:
            with conexoes_iq_lock:
                conexoes_iq_conectando.discard(usuario_id)

    threading.Thread(target=_conectar_em_fundo, daemon=True).start()
    return None
...
@app.route("/trocar-conta-iq", methods=["POST"])
@login_required
def trocar_conta_iq():
    """Troca a conta ativa do usuário entre demo (PRACTICE) e real (REAL) —
    sem precisar recadastrar email/senha, só um toggle rápido no dashboard.
    Se já existe uma conexão aberta pra esse usuário, troca o saldo ativo
    direto nela (instantâneo); senão, só salva a preferência — a próxima
    operação já conecta com o tipo certo."""
    dados = request.get_json(silent=True) or {}
    tipo = dados.get("tipo", "")
    if tipo not in ("PRACTICE", "REAL"):
        return jsonify({"sucesso": False, "erro": "tipo inválido (use 'PRACTICE' ou 'REAL')"}), 400

    if not current_user.tem_conta_iq_cadastrada():
        return jsonify({"sucesso": False, "erro": "Você ainda não cadastrou sua conta IQ Option."}), 400

    current_user.iq_conta_tipo = tipo
    db.session.commit()

    with conexoes_iq_lock:
        conexao = conexoes_iq_usuarios.get(current_user.id)
        if conexao is not None:
            try:
                if conexao.check_connect():
                    conexao.change_balance(tipo)
            except Exception as e:
                return jsonify({"sucesso": False, "erro": "Salvei a preferência, mas não consegui trocar na conexão ativa: " + str(e)}), 500

    return jsonify({"sucesso": True, "tipo": tipo})


@app.route("/status-conta")
@login_required
def status_conta():
    """Devolve o saldo atual da conta IQ Option do usuário logado (na
    conta DEMO ou REAL, o que estiver ativo) e se o ativo pedido está
    aberto pra operar agora. Usado pelo dashboard pra mostrar saldo e
    status do ativo sem precisar tentar enviar uma ordem só pra descobrir.
    NUNCA espera um login novo acontecer (usa a versão não-bloqueante) —
    essa rota é chamada automaticamente e com frequência, então não pode
    travar o servidor esperando a IQ Option responder."""
    ativo = request.args.get("ativo", "")

    try:
        conexao = obter_conexao_iq_sem_bloquear(current_user)
    except Exception as e:
        return jsonify({"sucesso": False, "erro": str(e)}), 400

    if conexao is None:
        return jsonify({"sucesso": True, "conectando": True, "saldo": None, "aberto": None,
                         "conta_tipo": current_user.iq_conta_tipo, "ativo": ativo})

    saldo = None
    try:
        saldo = conexao.get_balance()
    except Exception as e:
        print("get_balance falhou:", e)

    aberto = None
    if ativo:
        try:
            todos = conexao.get_all_open_time()
            info = (todos.get("turbo", {}).get(ativo)
                    or todos.get("binary", {}).get(ativo)
                    or todos.get("digital", {}).get(ativo))
            if info:
                aberto = bool(info.get("open"))
        except Exception as e:
            print("get_all_open_time falhou:", e)

    return jsonify({
        "sucesso": True,
        "saldo": saldo,
        "conta_tipo": current_user.iq_conta_tipo,
        "ativo": ativo,
        "aberto": aberto,
    })
...
@app.route("/resultado-ordem")
@login_required
def resultado_ordem():
    """Consulta se uma ordem já expirada deu ganho ou perda. Só chamar
    DEPOIS que o tempo de expiração já passou de verdade — check_win_v4
    fica esperando (bloqueando a requisição) até a IQ Option confirmar o
    resultado pelo WebSocket, então chamar cedo demais deixa a resposta
    lenta sem necessidade."""
    id_ordem = request.args.get("id", type=int)
    if not id_ordem:
        return jsonify({"sucesso": False, "erro": "id da ordem não informado"}), 400
    ...
    try:
        status, valor = conexao.check_win_v4(id_ordem)
    except Exception as e:
        return jsonify({"sucesso": False, "erro": "Erro ao consultar resultado: " + str(e)}), 500
    ...
    return jsonify({"sucesso": True, "resultado": resultado, "valor": valor})
